...
public class GLRotatingCubeFrame implements Application {
    private static final Log log = LogFactory.getLog(GLRotatingCubeFrame.class);
    ...
    public GLRotatingCubeFrame(AboutFrame aboutFrame) {
        af = aboutFrame;
    ...
    private void addCanvasToFrame(GLCanvas canvas, final Animator anim) {
        JFrame f = new JFrame("Über dieses Programm");
...
//        boolean enableLightsAtEnd = false;
    ...
    class RotatingSquareListener implements GLEventListener {
        ...
        private void textRenderer(GLAutoDrawable drawable, float angleZ) {
            ...
            renderer.draw3D(APPLICATION_NAME + " Version: " + APPLICATION_VERSION,
                  textXCoords - 20, angleZ, 0.1f, 1.0f);
            renderer.draw3D("Hibernate-Programmierung", textXCoords - 20, angleZ - 70,
                  0.1f, 1.0f);
            renderer.draw3D("Datenbank", textXCoords - 20, angleZ - 120, 0.1f, 1.0f);
            renderer.draw3D("GUI-Programmierung", textXCoords - 20, angleZ - 170, 0.1f,
                  1.0f);
            renderer.draw3D("Animationen & Grafik", textXCoords - 20, angleZ - 220, 0.1f,
                  1.0f);
            renderer.draw3D("Musik", textXCoords - 20, angleZ - 270, 0.1f, 1.0f);
            //renderer.flush();
            renderer.endRendering();
        }

        private void textNamesRenderer(GLAutoDrawable drawable, float angleX,
                                       float angleZ) {
            renderer1.beginRendering(drawable.getWidth(), drawable.getHeight());
            renderer1.setColor(new Color(255 - cFloatR, 255, 255 - cFloatG));
            renderer1.draw3D("Roman Stumm", textXCoords, angleZ - 90, 0.1f, 1.0f);
            renderer1.draw3D("Roman Stumm", textXCoords, angleZ - 140, 0.1f, 1.0f);
            renderer1.draw3D("Stephan Pudras", textXCoords, angleZ - 190, 0.1f, 1.0f);
            renderer1.draw3D("Stephan Pudras", textXCoords, angleZ - 240, 0.1f, 1.0f);
            renderer1.draw3D(AppStartup.getProperty("about.song.title"), textXCoords,
                  angleZ - 290, 0.1f, 1.0f);
            renderer1.draw3D(AppStartup.getProperty("about.song.copyright"), textXCoords,
                  angleZ - 310, 0.1f, 1.0f);
            //renderer1.flush();
        ...
        private void textRendererFooter(GLAutoDrawable drawable, float angleZ) {
            ...
            renderer2
                  .draw3D("www.elusivenotion.com", textXCoords, angleZ - 450, 0.1f, 1.0f);
            renderer2.draw3D(APPLICATION_NAME + " Version: " + APPLICATION_VERSION,
                  textXCoords, angleZ - 530, 0.1f, 1.0f);
            //renderer1.flush();
